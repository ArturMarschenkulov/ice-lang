...
struct Then<Ret(Arg)> {
    ...
    static Result<T, E> then(const Result<T, E>& result, Func func) {
        if (result.isOk()) { func(result.storage().template get<T>()); }
        return result;
    }
};
...
struct Wise<Ret(Arg)> {
    ...
    static Result<T, E> otherwise(const Result<T, E>& result, Func func) {
        ...
        if (result.isErr()) { func(result.storage().template get<E>()); }
        return result;
    }
};
...
struct Result {
    ...
    ~Result() {
        if (ok_) storage_.destroy(details::ok_tag());
        else storage_.destroy(details::err_tag());
    }
    ...
    typename std::enable_if<!std::is_same<U, void>::value, U>::type unwrapOr(const U& defaultValue) const {
        if (isOk()) { return storage().template get<U>(); }
        return defaultValue;
    }
    ...
    E unwrapErr() const {
        if (isErr()) { return storage().template get<E>(); }
...
bool operator==(const Result<T, E>& lhs, const Result<T, E>& rhs) {
    ...
    if (lhs.isOk() && rhs.isOk()) { return lhs.storage().template get<T>() == rhs.storage().template get<T>(); }
    if (lhs.isErr() && rhs.isErr()) { return lhs.storage().template get<E>() == rhs.storage().template get<E>(); }
}
