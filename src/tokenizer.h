...
public:
    ...
    CharCursor(const std::string& str) { m_it = &str[0]; }
...
public:
    ...
    auto create_token_from_type(const Token::TYPE type, CharCursor& start_cursor, CharCursor& current_cursor) const
        -> const Token;
    ...
    auto lex_char(CharCursor& current_cursor) const -> const Token::TYPE;
